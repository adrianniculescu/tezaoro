...
const Exchange = () => {
  ...
  console.log('Exchange: Before useChangellyExchange hook');
  
  try {
    const { loading, error, getCurrencies, getExchangeAmount } = useChangellyExchange();
    console.log('Exchange: useChangellyExchange hook successful');
    ...
    console.error('Exchange: Error in useChangellyExchange hook:', hookError);
    
    // Fallback render in case of hook error
    return (
      <PageLayout title="Crypto Exchange">
        <PageHeader 
          title="Instant Crypto Exchange" 
